...
/*
============================================================
    	CMSimple Plugin - Newsletter for CMSimple
============================================================
Version:    2.4.2  
Copyright:  © simpleSolutions 2010-2011
...
function newsletter($newspage_list)
{
	...
	$newspages=array();
	$o='<!-- CMSimple plugin Newsletter (c) simpleSolutions (//simplesolutions.dk) --><fieldset class="newsleter_fieldset"><legend class="newsleter_legend">'.( (isset($_GET['uns']))?'&nbsp;':$plugin_tx['newsletter']['subscribe'].'/').$plugin_tx['newsletter']['unsubscribe'].'&nbsp;</legend>';
	if ($adddel!='') {
...
function newsletter_create_form($newspage_list, $subscribermail, $subscriberfield, $newspages) {
	...
	$o.=tag("br").'<span class="newsletter_label">'.$plugin_tx['newsletter']['subscriber_email_label'].':&nbsp;</span>'.tag('input name="subscribermail" id="subscribermail" type="text" class="newsletter_inputfield" value="'.(isset($_GET['uns']) ? newsleter_convert($_GET['uns'],0) :		 	$subscribermail).'" onblur="newsletter_ValidEmail(document.subscribe);"');
	...
	if (!isset($_GET['uns']) && trim($plugin_tx['newsletter']['subscriber_fields_label'])!="") {
		...
		$user_input="";
		for ($i=0; $i<sizeof($labels); $i++) {
			$user_input .= '<span class="newsletter_label">'. trim($labels[$i]).':&nbsp;</span>'.tag('input name="'.rswu(trim($labels[$i])).'" id="'.rswu(trim($labels[$i])).'" type="text" class="newsletter_inputfield" value="'.$subscriberfield[$i].($mandatory?'" onblur="newsletter_EmptyField(this);"':'"'));
		}	
	}		
...
//echo 	"newspage_list=[$newspage_list] count=".count($newspage_arr);
	if (count($newspage_arr)>1) { // multiple newsletter subscribe, create checkboxes
		...
		$o.= ((isset($_GET['uns']))?$plugin_tx['newsletter']['select_newsletters_unsubscribe']:$plugin_tx['newsletter']['select_newsletters_subscribe']).tag('br');
		foreach ($newspage_arr as $np) {
			...
			$o.= tag('br').'<span class="newsletter_label">'.trim($np).':&nbsp;</span>'.tag('input name="news_page[]" type="checkbox"'.($checked?'checked="yes"':'').' value="'.rswu(trim($np)).'"');
		}
		$o.=tag('br');
	}
	else { // singel newsletter subscribe
		$o.=tag('input name="single_newspage" type="hidden" value="'.rswu($newspage_list).'"').'&nbsp;';	
	}
	$o.=tag('br').'<span class="newsletter_label"></span>'.tag('input type="submit" class="submit" value="'.$plugin_tx['newsletter']['send'].'"').'</form>';
	$o.=tag('br');
...
{
  global $plugin_cf;
	// Do not change this code unless you have paid for the license. 
	// To purchase the license visit https://simplesolutions.dk
  if ($plugin_cf['newsletter']['license'] =="Linkware") {    
  	if ($nl_type=="html") {
  		return '<hr><p>Powered by CMSimple plugin: "Newsletter"<br>&copy; <a href="https://simplesolutions.dk"><span style="color: #aaa;">simple</span><span style="color:red";>Solutions</span></a>.<p>';
  	}
  	else {
  		return '
  		______________________________________________________________
  		Powered by CMSimple plugin: "Newsletter"
  		(c) simpleSolutions (https://simpleSolutions.dk).';
  	}
  }
  else return "";    
// Do not change this code unless you have paid for the license. 
		...
		foreach ($newspages as $np) {
			...
			if ($fh=fopen($pth['folder']['plugins']."newsletter/data/subscribe_".rswu($np).".txt", "w")) {
				...
				if (fwrite($fh,$subscriber_str."\r\n")) { // write new subscriber
					$subscribe_msg.=$np.": ".$plugin_tx['newsletter']['subscribe_succes'].tag('br');
				}  
				else {
					$subscriber_class ='newsletter_subscribe_errmsg';
...
function newsletter_RemoveSubscriber($newspages, $subscribermail, $subscriberfield ) {
	...
	$removed_msg='<div class="'.$subscriber_class.'">'.$removed_msg.tag("br").tag("br").'</div>';
	return $removed_msg;
}
...
function newsletter_subscription_mail($subscribermail, $subject, $msg, $link)
{
  ...
  if (trim($plugin_cf['newsletter']['smtp'])!="") {
	...
	$link_href="";
	if ($link!="")
		$link_href='<a href="'.$link.'">'.$link.'</a>'.tag('br');
//var_dump ($confirmation_template_file);	
	$template=preg_replace('/\{CONTENT\}/',$msg.tag('br').$link_href,$template);
	$template=preg_replace('/<.*\/.*body.*>/i',tag('br').newsletter_license("html")."</body>",$template);
//var_dump($template);	
	
	$mail->MsgHTML($template);
	if(!$mail->Send())
		return " ".$plugin_tx['newsletter']['subscribe_confirmation_mail_error']." (".$mail->ErrorInfo.")".tag("br");
	else
